...
export class CalendarTableComponent implements OnInit {
  ...
  constructor(
    ...
    this.dateService.switchMonth$.subscribe((date) => {
      ...
      this.userService.updateVacations(this.currentDate);
      this.userService.updateVacationById(this.currentDate);
      this.generateHeaderArray();
    });
  }
  ...
  generateHeaderArray(): void {
    for (let index = 0; index < this.daysInMonth; index++) {
      ...
      const curDate = moment(this.currentDate).add(index, 'days').format('ddd');
      const curFullDate = moment(this.currentDate)
        .add(index, 'days')
        .format('YYYY-MM-DD');
      this.days[index] = {
