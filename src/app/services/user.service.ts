...
export class UserService {
  ...
  vacationInfo: BehaviorSubject<number[]> = new BehaviorSubject<number[]>([]);
  vacationInfoById: BehaviorSubject<UserVacationsById> = new BehaviorSubject<UserVacationsById>({});
  ...
  updateVacations(curDate: string): void {
  ...
  getVacations(curDate: string): Observable<number[]> {
    this.updateVacations(curDate);
    return this.vacationInfo.asObservable();
  }
  updateVacationById(curDate: string) {
    this.vacations = {};
    const curr = moment(curDate);
    for (const team of this.teams) {
      for (const member of team.members) {
  ...
  getVacationById(curDate: string): Observable<UserVacationsById> {
    this.updateVacationById(curDate);
    return this.vacationInfoById.asObservable();
  }
}
